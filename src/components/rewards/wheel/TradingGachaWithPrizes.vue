        ...
        >
          <div
            ...
            :style="{
              ...
              '--i': i,
              '--src': `url('${encodeImageUrl(prize.imageOnGacha || prize.image)}')`
            }"
          />
        </div>
      </div>
...
import { useTradingGacha } from '@/composables/use-wheel-pulls'
import type { TradingGachaPrize } from '@/composables/use-wheel-pulls'
...
const wheelSections = ref<TradingGachaPrize[]>([])

// LOAD SECTIONS
watchEffect(() => {
  wheelSections.value =
    currentGacha.value?.prizes
      .slice()
...
// ❗ THIS WAS WRONG EARLIER – MUST MATCH SPINS WHEEL
